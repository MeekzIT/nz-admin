...
import { fetchEditAppartementData, fetchGetBuildingSchemaData } from '../../../redux/slices/homeBuilding/fetchService'
import { filterAppartementsFromFloor, handleChangeTextFildsAppartement } from '../../../redux/slices/homeBuilding'
import { Box, FormControl, InputLabel, Select, MenuItem, Accordion, AccordionDetails, AccordionSummary, Button, Typography, TextField, Switch } from '@mui/material'
...
const BuildingHomePage = () => {
    ...
    const { appartements, loading, floors, appartementsInFloor } = useAppSelector((state) => state.bomeBuilding)
    ...
    return (
        <>
                ...
                <FormControl fullWidth>
                    ...
                    >
                        {floors?.length && floors.map((floor) => {
                            return (
                                <MenuItem value={floor.floor} key={floor.floor}>{floor.floor}-րդ Հարկ</MenuItem>
                            )
                        })}
                    </Select>
                </FormControl>
            ...
            <Box>
                {appartementsInFloor?.map(({ AppartementData, price, in_stock, id }, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <AccordionDetails>
                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    ...
                                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                                        ...
                                        <TextField
                                            ...
                                            variant="outlined"
                                            value={price}
                                            onChange={(event) => dispatch(handleChangeTextFildsAppartement({ id, key: "price", text: event.target.value }))}
                                        />
                                    </Box>
                                    <Box>
                                        ...
                                        <Switch
                                            ...
                                            checked={in_stock}
                                            onChange={(event) => {
                                                dispatch(handleChangeTextFildsAppartement({ id, key: "in_stock", booleanState: event.target.checked }));
                                            }}
                                        />
                                    </Box>
                                </Box>
                                ...
                                >
                                    <AccordionSummary
                                        ...
                                        id="panel2-header"
                                    >
                                        <Typography component="span">լրացուցիչ տեղեկություններ բնակարանի մասին։</Typography>
                                    </AccordionSummary>
                                    <AccordionDetails>
                            ...
                            <Button
                                sx={{ float: "right", mt: 2 }}
                                variant='contained'
                                onClick={() => dispatch(fetchEditAppartementData({ id, data: { price, in_stock } }))}
                            >
