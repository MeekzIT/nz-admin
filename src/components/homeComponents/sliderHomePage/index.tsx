...
import {
  ...
  fetchGetSliderData,
  fetchGetSlidersData,
} from "../../../redux/slices/homeSliderSlice/fetchService";
...
const SliderHomePage = () => {
  ...
  const { sliderNames, currentSlider, loading, createSliderData } =
  ...
  const saveDataHandler = () => {
    if (currentSliderID && currentSlider) {
      dispatch(
        fetchEditSlider({ id: Number(currentSliderID), data: currentSlider })
      );
    }
  };
  ...
  const openModalQuestionForDeleteContact = () => {
    ...
    dispatch(
      setTitleForQuestionModal({
        titleModal: "Դուք իսկապես ցանկանում եք ջնջել սլայդերը?",
      })
    );
      ...
      >
        <Box m={2}>
          ...
          <TapsForChangeImagesComponent
            ...
            forCreateFlag
          />
          <Button variant="contained" onClick={createNewSlider}>
            Պահպանել
          </Button>
        </Box>
      </BaseModal>
