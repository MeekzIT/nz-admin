...
} from "../../redux/slices/projectsSlice/fetchService";
import {
...
import {
  imagesDataForMapInTapsComponent,
  textDataForMapInTapsComponent,
} from "./constants";
...
const ProjectsPage = () => {
  ...
  const { projectsNames, loading, currentProject, createProectdata } =
    dataProjects;
  ...
  const saveDataHandler = () => {
    if (currentProject) {
      dispatch(
        fetchChangeInfoProjects({
          productId: Number(currentProjectId),
          data: currentProject,
        })
      );
    }
  ...
  const openModalQuestionForDeleteContact = () => {
    ...
    dispatch(
      setTitleForQuestionModal({
        titleModal: "Դուք իսկապես ցանկանում եք ջնջել նախագիծը?",
      })
    );
  ...
  return (
    <>
      ...
      <Button
        sx={{ textAlign: "right", mb: 5 }}
        variant="contained"
        onClick={() => setIsOpenCreateProjectModal(!isOpenCreateProjectModal)}
      >
        Ավելացրեք նախագիծ
      </Button>
        ...
        <Container>
          <BlockWithBackgroundBlend>
            <TapsForChangeTextComponent
              ...
              tabDataForMap={textDataForMapInTapsComponent}
              dataForChange={currentProject}
            />
          </BlockWithBackgroundBlend>
          <BlockWithBackgroundBlend>
            <PageTitle>Լուսանկարներ</PageTitle>
            <TapsForChangeImagesComponent
              tabDataForMap={imagesDataForMapInTapsComponent}
              dataForChange={currentProject}
              onChangeImageInObject={handleChangeProjectData}
            />
          </BlockWithBackgroundBlend>
          <BlockWithBackgroundBlend sx={{ display: "flex", gap: 2 }}>
            ...
            <Button
              variant="contained"
              onClick={openModalQuestionForDeleteContact}
              sx={{ p: 2, width: "100%" }}
              color="error"
            >
              Ջնջել նախագիծը
      ...
      >
        <Box m={2}>
          <TapsForChangeTextComponent
            handleChangeProjectData={handleChangeProjectData}
            tabDataForMap={textDataForMapInTapsComponent}
            dataForChange={createProectdata}
            forCreateFlag
          />
          <TapsForChangeImagesComponent
            tabDataForMap={imagesDataForMapInTapsComponent}
